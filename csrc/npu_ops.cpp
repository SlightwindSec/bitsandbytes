...
#include "aclrtlaunch_row_col_dequant.h"
...
namespace get_row_col_dequant_tiling {
    const uint32_t MIN_LEN_ALIGNED = 16; // align unit in cache 32B
    const uint32_t DEFAULT_BUFFER_NUM = 2;

    struct DequantizeTilingCalculator : public DequantizeTilingData {
    public:
        bool CalcTiling(int numRows, int numCols, uint8_t shapeSize, uint32_t totalCore, uint64_t ubSize);
    private:
        inline void CalcTilingForNoAlign(uint64_t ubSize);
        inline void CalcTilingForAlign(uint64_t ubSize);
        inline bool CalcTileColMax(uint64_t ubSize, uint16_t bufferNum);
        inline uint32_t CalcTileRowMaxByCol(uint64_t ubSize, uint16_t bufferNum, uint64_t tileCol);
        inline void CalcBlockTilingParam(uint64_t ubSize, uint32_t tileNum, BlockTilingParam& blockTilingParam);

        uint32_t tileColMax = 0;
        uint32_t totalAvailableCore = 0;
    };

    inline bool DequantizeTilingCalculator::CalcTileColMax(uint64_t ubSize, uint16_t bufferNum)
    {
        tileColMax = ALIGNDOWN((ubSize - 32) / (12 * bufferNum + 8), L2_CACHE_LINE_SIZE / 4);
        return true;
    }

    inline uint32_t DequantizeTilingCalculator::CalcTileRowMaxByCol(uint64_t ubSize, uint16_t bufferNum, uint64_t tileCol)
    {
        uint32_t maxTileRow = (ubSize  - 6 * bufferNum * tileCol - 64 * bufferNum) / (6 * bufferNum  * tileCol + 4 * bufferNum + 12 * tileCol);
        maxTileRow = ALIGNDOWN(maxTileRow, L2_CACHE_LINE_SIZE / 4);
        return maxTileRow;
    }

    inline void DequantizeTilingCalculator::CalcBlockTilingParam(uint64_t ubSize, uint32_t tileNum, BlockTilingParam& blockTilingParam)
    {
        uint16_t bufferNum = isDoubleBuffer ? DEFAULT_BUFFER_NUM : 1;
        uint32_t maxTileRow = CalcTileRowMaxByCol(ubSize, bufferNum, N);
        uint32_t totalBlockRow = tileNum * MIN_LEN_ALIGNED;
        while(totalBlockRow > maxTileRow){
            blockTilingParam.loopNum += 1;
            blockTilingParam.tileRow = maxTileRow;
            totalBlockRow -= maxTileRow;
        }

        if (totalBlockRow > 0) {
            blockTilingParam.isTailBlockExist = 1;
            blockTilingParam.tailRow = ALIGNUP(totalBlockRow, MIN_LEN_ALIGNED);
        }
    }

    inline void DequantizeTilingCalculator::CalcTilingForNoAlign(uint64_t ubSize){
        is32BAligned = 0;

        if (M < MIN_LEN_ALIGNED) {
            usedCoreNum = 1;
            baseBlockNum = 1;
            baseBlockTilingParam.loopNum = 1;
            baseBlockTilingParam.totalRow = ALIGNUP(M, MIN_LEN_ALIGNED);
            baseBlockTilingParam.tileRow = ALIGNUP(M, MIN_LEN_ALIGNED);
        } else {
            uint32_t tileNum = M / MIN_LEN_ALIGNED;
            if (tileNum * MIN_LEN_ALIGNED != M) {
                tileNum += 1;
            }

            usedCoreNum = std::min(totalAvailableCore, tileNum);

            if (usedCoreNum < totalAvailableCore) {
                baseBlockNum = usedCoreNum;
                baseBlockTilingParam.loopNum = 1;
                baseBlockTilingParam.totalRow = MIN_LEN_ALIGNED;
                baseBlockTilingParam.tileRow = MIN_LEN_ALIGNED;
            } else{
                uint32_t tailBlockTileNum = tileNum / totalAvailableCore;
                uint32_t baseBlockTileNum = tailBlockTileNum + 1;
                baseBlockNum = tileNum - totalAvailableCore * tailBlockTileNum;
                baseBlockTilingParam.totalRow = baseBlockTileNum * MIN_LEN_ALIGNED;
                tailBlockTilingParam.totalRow = tailBlockTileNum * MIN_LEN_ALIGNED;

                CalcBlockTilingParam(ubSize, baseBlockTileNum, baseBlockTilingParam);
                CalcBlockTilingParam(ubSize, tailBlockTileNum, tailBlockTilingParam);
            }
        }
    }

    inline void DequantizeTilingCalculator::CalcTilingForAlign(uint64_t ubSize){
        is32BAligned = 1;
        uint16_t buffer_num = isDoubleBuffer ? DEFAULT_BUFFER_NUM : 1;
        CalcTileColMax(ubSize, buffer_num);
        ...
        if (N < tileColMax) {
            tileNum = M;
            baseTileNum = M;
            baseTileCol = ALIGNDOWN(N, MIN_LEN_ALIGNED);
            tailTileCol = ALIGNUP(N % baseTileCol, MIN_LEN_ALIGNED);
        } else{
            tileNum = N / tileColMax * M;
            baseTileNum = tileNum;
            baseTileCol = tileColMax;
            tailTileCol = ALIGNUP(N % baseTileCol, MIN_LEN_ALIGNED);
        }
        ...
        if (usedCoreNum < totalAvailableCore) {
            ...
            if (tileNum % usedCoreNum == 0) {
                baseCoreCalcNum = tileNum / usedCoreNum;
                baseBlockNum = usedCoreNum;
            } else{
                tailCoreCalcNum = tileNum / usedCoreNum;
                baseBlockNum = tileNum - usedCoreNum * tailCoreCalcNum;
                baseCoreCalcNum = tailCoreCalcNum + 1;
            }
        }
    }

    bool DequantizeTilingCalculator::CalcTiling(int numRows, int numCols, uint8_t shapeSize, uint32_t totalCore, uint64_t ubSize)
    {
        totalAvailableCore = totalCore;

        if (shapeSize == 2) {
            int32_t inShape0 = numRows;
            int32_t inShape1 = numCols;
            M = inShape0;
            N = inShape1;
        } else {
            printf("CalcTiling: Unsupported dimNum: %d.\n", shapeSize);
            return false;
        }
        ...
        if (N < 16) {
            CalcTilingForNoAlign(ubSize);
        } else{
            CalcTilingForAlign(ubSize);
        }
        return true;
    }

    uint32_t TilingCalcFunc(int numRows, int numCols, uint8_t shapeSize, get_row_col_dequant_tiling::DequantizeTilingCalculator *tiling)
    {
        uint32_t totalCore = 40;
        uint64_t ubSize = 192 * 1024;
        if (totalCore < 0 || totalCore >= MAX_CORE_NUMBER || ubSize <= UB_RESERVED_BUFF) {
            printf("Compile Info is invalid, coreNum:%u, ubSize:%lu\n", totalCore, ubSize);
            return 1;
        ...
        if (!tiling->CalcTiling(numRows, numCols, shapeSize, totalCore, ubSize)) {
            return 1;
        }
        return 0;
    }
...
void rowColDequant(uint8_t *A, uint8_t *rowStats, uint8_t *colStats, uint8_t *out, uint8_t *bias, int numRows, int numCols, void* stream) {
    uint8_t shapeSize = 2;
    uint32_t blockDim = 40;
    size_t tilingSize = sizeof(struct get_row_col_dequant_tiling::DequantizeTilingCalculator);
    get_row_col_dequant_tiling::DequantizeTilingCalculator *tilingHost;
    tilingHost = (struct get_row_col_dequant_tiling::DequantizeTilingCalculator *)malloc(sizeof(struct get_row_col_dequant_tiling::DequantizeTilingCalculator));
    uint32_t error = get_row_col_dequant_tiling::TilingCalcFunc(numRows, numCols, shapeSize, tilingHost);
    if (error != 0) {
        printf("An error occurred.\n");
    }
    uint8_t *tilingDevice = nullptr;
    aclrtMalloc((void **)&tilingDevice, tilingSize, ACL_MEM_MALLOC_NORMAL_ONLY);
    aclrtMemcpyAsync((void *)tilingDevice, tilingSize, tilingHost, tilingSize, ACL_MEMCPY_HOST_TO_DEVICE, stream);
    ACLRT_LAUNCH_KERNEL(row_col_dequant)(blockDim, stream, A, rowStats, colStats, bias, out, tilingDevice);
    aclrtFree(tilingDevice);
}
