...
#define ROW_COL_QUANT_PROCESS(kernelTiling)                                                       \
    ...
    {
        DataCopyParams copyInParams = {copyParam.blockCount,
                                       copyParam.blockLen,
                                       copyParam.stride,
                                       0};
    ...
    template<typename InType, typename CalType, typename OutType>
    __aicore__ inline void RowColQuantKernel<InType, CalType, OutType>::ComputeForOutlier(uint32_t offset, uint32_t calcLen,
                                                                                        LocalTensor<int32_t>& outlierIdx, uint32_t& outlierNum)
    {
...
namespace row_col_dequant_kernel {
    struct DequantizeTilingKernel {
        DequantizeTilingData tilingData;
        BlockTilingParam blockTilingParam;
        ...
        __aicore__ inline void init(GM_ADDR tiling_gm) {
            auto tempTilingGm = (__gm__ DequantizeTilingData*)tiling_gm;
            blockId = AscendC::GetBlockIdx();
            tilingData.M = tempTilingGm->M;
            tilingData.N = tempTilingGm->N;
            tilingData.usedCoreNum = tempTilingGm->usedCoreNum;

            tilingData.isBias = tempTilingGm->isBias;
            tilingData.is32BAligned = tempTilingGm->is32BAligned;
            tilingData.isDoubleBuffer = tempTilingGm->isDoubleBuffer;
            tilingData.baseBlockNum = tempTilingGm->baseBlockNum;

            tilingData.baseBlockTilingParam.totalRow = tempTilingGm->baseBlockTilingParam.totalRow;
            tilingData.baseBlockTilingParam.loopNum = tempTilingGm->baseBlockTilingParam.loopNum;
            tilingData.baseBlockTilingParam.tileRow = tempTilingGm->baseBlockTilingParam.tileRow;
            tilingData.baseBlockTilingParam.tileCol = tempTilingGm->baseBlockTilingParam.tileCol;
            tilingData.baseBlockTilingParam.isTailBlockExist = tempTilingGm->baseBlockTilingParam.isTailBlockExist;
            tilingData.baseBlockTilingParam.tailRow = tempTilingGm->baseBlockTilingParam.tailRow;

            tilingData.tailBlockTilingParam.totalRow = tempTilingGm->tailBlockTilingParam.totalRow;
            tilingData.tailBlockTilingParam.loopNum = tempTilingGm->tailBlockTilingParam.loopNum;
            tilingData.tailBlockTilingParam.tileRow = tempTilingGm->tailBlockTilingParam.tileRow;
            tilingData.tailBlockTilingParam.tileCol = tempTilingGm->tailBlockTilingParam.tileCol;
            tilingData.tailBlockTilingParam.isTailBlockExist = tempTilingGm->tailBlockTilingParam.isTailBlockExist;
            tilingData.tailBlockTilingParam.tailRow = tempTilingGm->tailBlockTilingParam.tailRow;

            tilingData.baseTileNum = tempTilingGm->baseTileNum;
            tilingData.baseTileCol = tempTilingGm->baseTileCol;
            tilingData.tailTileCol = tempTilingGm->tailTileCol;
            tilingData.baseCoreCalcNum = tempTilingGm->baseCoreCalcNum;
            tilingData.tailCoreCalcNum = tempTilingGm->tailCoreCalcNum;

            if (tilingData.is32BAligned == 0) {
                calcNonAlignModeOffset();
            }
        }

        __aicore__ inline void calcNonAlignModeOffset() {
            if (blockId < tilingData.baseBlockNum) {
                blockTilingParam = tilingData.baseBlockTilingParam;
                rowOffset = blockId * blockTilingParam.totalRow;
                if (blockId == (tilingData.usedCoreNum - 1)) {
                    if (tilingData.M >= blockTilingParam.totalRow) {
                        rowOffset = tilingData.M - blockTilingParam.totalRow;
                    }
                }
            } else {
                blockTilingParam = tilingData.tailBlockTilingParam;
                if (blockId == (tilingData.usedCoreNum - 1)) {
                    rowOffset = tilingData.M - blockTilingParam.totalRow;
                } else {
                    rowOffset = tilingData.baseBlockNum * tilingData.baseBlockTilingParam.totalRow +
                                (blockId - tilingData.baseBlockNum) * blockTilingParam.totalRow;
                }
            }
        }

        __aicore__ inline void calcAlignModeOffset(uint32_t tileIdx) {
            uint32_t trueTileIdx;
            if (blockId < tilingData.baseBlockNum) {
                trueTileIdx = blockId * tilingData.baseCoreCalcNum + tileIdx;
            } else {
                trueTileIdx = tilingData.baseBlockNum * tilingData.baseCoreCalcNum +
                              (blockId - tilingData.baseBlockNum) * tilingData.tailCoreCalcNum + tileIdx;
            }
            rowOffset = trueTileIdx % tilingData.M;
            if (trueTileIdx < tilingData.baseTileNum){
                datalen = tilingData.baseTileCol;
                colOffset = trueTileIdx / tilingData.M * tilingData.baseTileCol;
            } else{
                datalen = tilingData.tailTileCol;
                colOffset = tilingData.N - tilingData.tailTileCol;
            }
        }

        __aicore__ inline uint32_t getTileNum() {
            return blockId < tilingData.baseBlockNum ? tilingData.baseCoreCalcNum : tilingData.tailCoreCalcNum;
        }
    };

    // DequantizeVectorAligned
    template<uint16_t bufferNum>
    class DequantizeVectorAligned {
    public:
        __aicore__ inline DequantizeVectorAligned() {}
        __aicore__ inline ~DequantizeVectorAligned() {}
        __aicore__ inline void Init(GM_ADDR ptrA_gm, GM_ADDR ptrRow_gm, GM_ADDR ptrCol_gm, GM_ADDR bias_gm,
                                    GM_ADDR outA_gm, GM_ADDR tiling_gm);
        __aicore__ inline void Process();
    private:
        ...
        TPipe pipe;
        TQue<QuePosition::VECIN, bufferNum> inQueuePtrA;
        TQue<QuePosition::VECIN, bufferNum> inQueueRowMax;
        TQue<QuePosition::VECIN, bufferNum> inQueueColMax;
        TQue<QuePosition::VECIN, bufferNum> inQueueBias;
        TQue<QuePosition::VECOUT, bufferNum> outQueueOutA;
        TBuf<TPosition::VECCALC> aMatrixFloatBuf;
        TBuf<TPosition::VECCALC> biasFloatBuf;
        GlobalTensor<int32_t> ptrAGlobal;
        GlobalTensor<float> rowMaxGlobal;
        GlobalTensor<float> colMaxGlobal;
        GlobalTensor<half> biasGlobal;
        GlobalTensor<half> outAGlobal;
        GlobalTensor<float> newRowMaxGlobal;
        GlobalTensor<float> newColMaxGlobal;

        DequantizeTilingKernel tiling;
    };

    template<uint16_t bufferNum>
    __aicore__ inline void DequantizeVectorAligned<bufferNum>::Init(GM_ADDR ptrA_gm, GM_ADDR ptrRow_gm,
                                                                    GM_ADDR ptrCol_gm, GM_ADDR bias_gm,
                                                                    GM_ADDR outA_gm, GM_ADDR tiling_gm)
    {
        tiling.init(tiling_gm);
        // get start index for current core, core parallel
        ptrAGlobal.SetGlobalBuffer((__gm__ int32_t*)ptrA_gm, tiling.tilingData.M * tiling.tilingData.N);
        rowMaxGlobal.SetGlobalBuffer((__gm__ float*)ptrRow_gm, tiling.tilingData.M);
        colMaxGlobal.SetGlobalBuffer((__gm__ float*)ptrCol_gm, tiling.tilingData.N);
        outAGlobal.SetGlobalBuffer((__gm__ half*)outA_gm, tiling.tilingData.M * tiling.tilingData.N);

        // pipe alloc memory to queue, the unit is Bytes
        pipe.InitBuffer(inQueuePtrA, bufferNum, tiling.tilingData.baseTileCol * sizeof(int32_t));
        pipe.InitBuffer(inQueueRowMax, bufferNum, 32);
        pipe.InitBuffer(inQueueColMax, bufferNum, tiling.tilingData.baseTileCol * sizeof(float));
        pipe.InitBuffer(outQueueOutA, bufferNum, tiling.tilingData.baseTileCol * sizeof(half));

        pipe.InitBuffer(aMatrixFloatBuf, tiling.tilingData.baseTileCol * sizeof(float));

        if (tiling.tilingData.isBias == 1) {
            biasGlobal.SetGlobalBuffer((__gm__ half*)bias_gm, tiling.tilingData.N);
            pipe.InitBuffer(biasFloatBuf, tiling.tilingData.baseTileCol * sizeof(float));
            pipe.InitBuffer(inQueueBias, bufferNum, tiling.tilingData.baseTileCol * sizeof(half));
        }
    }

    template<uint16_t bufferNum>
    __aicore__ inline void DequantizeVectorAligned<bufferNum>::Process() {
        for (uint32_t idx = 0; idx < tiling.getTileNum(); idx++){
            tiling.calcAlignModeOffset(idx);
            CopyIn();
            Compute();
            CopyOut();
        }
    ...
    template<uint16_t bufferNum>
    __aicore__ inline void DequantizeVectorAligned<bufferNum>::CopyIn()
    {
        // Copy ptrA
        LocalTensor<int32_t> ptrALocal = inQueuePtrA.template AllocTensor<int32_t>();
        DataCopy(ptrALocal, ptrAGlobal[tiling.rowOffset * tiling.tilingData.N + tiling.colOffset], tiling.datalen);
        inQueuePtrA.template EnQue(ptrALocal);
        // Copy RowMax
        LocalTensor<float> rowMaxLocal = inQueueRowMax.template AllocTensor<float>();
        DataCopyParams copyParams{1, sizeof(float), 0, 0};
        DataCopyPadParams padParams{false, 0, 0, 0};
        DataCopyPad(rowMaxLocal, rowMaxGlobal[tiling.rowOffset], copyParams, padParams);
        inQueueRowMax.template EnQue(rowMaxLocal);
        // Copy ColMax
        LocalTensor<float> colMaxLocal = inQueueColMax.template AllocTensor<float>();
        DataCopy(colMaxLocal, colMaxGlobal[tiling.colOffset], tiling.datalen);
        inQueueColMax.EnQue(colMaxLocal);

        if (tiling.tilingData.isBias == 1) {
            LocalTensor<half> biasLocal = inQueueBias.template AllocTensor<half>();
            DataCopy(biasLocal, biasGlobal[tiling.colOffset], tiling.datalen);
            inQueueBias.template EnQue(biasLocal);
        }
    }

    template<uint16_t bufferNum>
    __aicore__ inline void DequantizeVectorAligned<bufferNum>::Compute()
    {
        LocalTensor<float> rowMaxLocal = inQueueRowMax.template DeQue<float>();
        LocalTensor<float> colMaxLocal = inQueueColMax.template DeQue<float>();

        set_flag(PIPE_V, PIPE_S, EVENT_ID0);
        wait_flag(PIPE_V, PIPE_S, EVENT_ID0);
        float rowMax = rowMaxLocal.GetValue(0);
        set_flag(PIPE_S, PIPE_V, EVENT_ID0);
        wait_flag(PIPE_S, PIPE_V, EVENT_ID0);
        pipe_barrier(PIPE_V);

        Muls(colMaxLocal, colMaxLocal, rowMax, tiling.datalen);
        pipe_barrier(PIPE_V);
        inQueueRowMax.template FreeTensor(rowMaxLocal);

        LocalTensor<int32_t> ptrALocal = inQueuePtrA.template DeQue<int32_t>();
        LocalTensor<float> ptrAFloattemp = aMatrixFloatBuf.Get<float>();
        Cast(ptrAFloattemp, ptrALocal, RoundMode::CAST_NONE, tiling.datalen);
        pipe_barrier(PIPE_V);

        Mul(ptrAFloattemp, ptrAFloattemp, colMaxLocal, tiling.datalen);
        pipe_barrier(PIPE_V);
        inQueueColMax.template FreeTensor(colMaxLocal);

        float factor = 1.0f / (127.0f * 127.0f);
        Muls(ptrAFloattemp, ptrAFloattemp, factor, tiling.datalen);
        pipe_barrier(PIPE_V);

        if (tiling.tilingData.isBias == 1) {
            LocalTensor<float> biasFloatLocal = biasFloatBuf.Get<float>();
            LocalTensor<half> biasLocal = inQueueBias.template DeQue<half>();

            Cast(biasFloatLocal, biasLocal, RoundMode::CAST_NONE, tiling.datalen);
            pipe_barrier(PIPE_V);

            inQueueBias.template FreeTensor(biasLocal);

            Add(ptrAFloattemp, ptrAFloattemp, biasFloatLocal, tiling.datalen);
            pipe_barrier(PIPE_V);
        }

        LocalTensor<half> outALocal = outQueueOutA.template AllocTensor<half>();
        Cast(outALocal, ptrAFloattemp, RoundMode::CAST_NONE, tiling.datalen);
        pipe_barrier(PIPE_V);

        outQueueOutA.template EnQue<half>(outALocal);
        inQueuePtrA.template FreeTensor(ptrALocal);
    }

    template<uint16_t bufferNum>
    __aicore__ inline void DequantizeVectorAligned<bufferNum>::CopyOut()
    {
        // copy OutA
        LocalTensor<half> outALocal = outQueueOutA.template DeQue<half>();
        DataCopy(outAGlobal[tiling.rowOffset * tiling.tilingData.N + tiling.colOffset], outALocal, tiling.datalen);
        outQueueOutA.template FreeTensor(outALocal);
    }

    // DequantizeVectorNonAligned
    template<uint16_t bufferNum>
    class DequantizeVectorNonAligned {
    public:
        __aicore__ inline DequantizeVectorNonAligned() {}
        __aicore__ inline ~DequantizeVectorNonAligned() {}
        __aicore__ inline void Init(GM_ADDR ptrA_gm, GM_ADDR ptrRow_gm, GM_ADDR ptrCol_gm, GM_ADDR bias_gm,
                                    GM_ADDR outA_gm, GM_ADDR tiling_gm);
        __aicore__ inline void Process();
    private:
        __aicore__ inline void CopyIn(uint32_t rowOffset, uint32_t rowCount, uint32_t dataLen);
        __aicore__ inline void Compute(uint32_t rowCount, uint32_t dataLen);
        __aicore__ inline void CopyOut(uint32_t rowOffset, uint32_t rowCount, uint32_t dataLen);
        ...
        TQue<QuePosition::VECOUT, bufferNum> outQueueOutA;
        TBuf<TPosition::VECCALC> aMatrixFloatBuf;
        TBuf<TPosition::VECCALC> rowColProductFloatBuf;
        TBuf<TPosition::VECCALC> biasFloatBuf;
        GlobalTensor<int32_t> ptrAGlobal;
        GlobalTensor<float> rowMaxGlobal;
        GlobalTensor<float> colMaxGlobal;
    ...
    template<uint16_t bufferNum>
    __aicore__ inline void DequantizeVectorNonAligned<bufferNum>::Init(GM_ADDR ptrA_gm, GM_ADDR ptrRow_gm,
                                                                       GM_ADDR ptrCol_gm, GM_ADDR bias_gm,
                                                                       GM_ADDR outA_gm, GM_ADDR tiling_gm)
    {
        tiling.init(tiling_gm);
        // get start index for current core, core parallel
        ptrAGlobal.SetGlobalBuffer((__gm__ int32_t*)ptrA_gm + tiling.rowOffset * tiling.tilingData.N, tiling.blockTilingParam.totalRow * tiling.tilingData.N);
        rowMaxGlobal.SetGlobalBuffer((__gm__ float*)ptrRow_gm + tiling.rowOffset, tiling.blockTilingParam.totalRow);
        colMaxGlobal.SetGlobalBuffer((__gm__ float*)ptrCol_gm, tiling.tilingData.N);
        outAGlobal.SetGlobalBuffer((__gm__ half*)outA_gm + tiling.rowOffset * tiling.tilingData.N, tiling.blockTilingParam.totalRow * tiling.tilingData.N);

        // pipe alloc memory to queue, the unit is Bytes

        uint64_t maxTileRow = tiling.blockTilingParam.tileRow == 0 ?  tiling.blockTilingParam.tailRow : tiling.blockTilingParam.tileRow;
        pipe.InitBuffer(inQueuePtrA, bufferNum, maxTileRow * tiling.tilingData.N * sizeof(int32_t));
        pipe.InitBuffer(inQueueRowMax, bufferNum, maxTileRow * sizeof(float));
        pipe.InitBuffer(inQueueColMax, bufferNum, tiling.tilingData.N * sizeof(float) / 32 * 32 + 32);
        pipe.InitBuffer(outQueueOutA, bufferNum, maxTileRow * tiling.tilingData.N * sizeof(half));

        pipe.InitBuffer(aMatrixFloatBuf, maxTileRow * tiling.tilingData.N * sizeof(float));
        pipe.InitBuffer(rowColProductFloatBuf, maxTileRow * tiling.tilingData.N * sizeof(float));

        if (tiling.tilingData.isBias == 1) {
            biasGlobal.SetGlobalBuffer((__gm__ half*)bias_gm, tiling.tilingData.N);

            pipe.InitBuffer(biasFloatBuf, maxTileRow * tiling.tilingData.N * sizeof(float));
            pipe.InitBuffer(inQueueBias, bufferNum, tiling.tilingData.N * sizeof(half) / 32 * 32 + 32);
        }
    }

    template<uint16_t bufferNum>
    __aicore__ inline void DequantizeVectorNonAligned<bufferNum>::Process() {
        uint32_t rowOffset;
        uint32_t rowCount;
        uint32_t dataLen;

        for (uint32_t idx = 0; idx < tiling.blockTilingParam.loopNum; idx++){
            rowOffset = tiling.blockTilingParam.tileRow * idx;
            rowCount = tiling.blockTilingParam.tileRow;
            dataLen = rowCount * tiling.tilingData.N;
            CopyIn(rowOffset, rowCount, dataLen);
            Compute(rowCount, dataLen);
            CopyOut(rowOffset, rowCount, dataLen);
        }
        if (tiling.blockTilingParam.isTailBlockExist == 1){
            rowOffset = tiling.blockTilingParam.totalRow - tiling.blockTilingParam.tailRow;
            rowCount = tiling.blockTilingParam.tailRow;
            dataLen = rowCount * tiling.tilingData.N;
            CopyIn(rowOffset, rowCount, dataLen);
            Compute(rowCount, dataLen);
            CopyOut(rowOffset, rowCount, dataLen);
    ...
    template<uint16_t bufferNum>
    __aicore__ inline void DequantizeVectorNonAligned<bufferNum>::CopyIn(uint32_t rowOffset, uint32_t rowCount, uint32_t dataLen)
    {
        // Copy ptrA
        LocalTensor<int32_t> ptrALocal = inQueuePtrA.template AllocTensor<int32_t>();
        DataCopy(ptrALocal, ptrAGlobal[rowOffset * tiling.tilingData.N], dataLen);
        inQueuePtrA.template EnQue(ptrALocal);
        // Copy RowMax
        LocalTensor<float> rowMaxLocal = inQueueRowMax.template AllocTensor<float>();
        DataCopy(rowMaxLocal, rowMaxGlobal[rowOffset], rowCount);
        inQueueRowMax.template EnQue(rowMaxLocal);
        // Copy ColMax
        LocalTensor<float> colMaxLocal = inQueueColMax.template AllocTensor<float>();
        DataCopyPadParams padParams{false, 0, 0, 0};
        uint16_t copyLen = tiling.tilingData.N * sizeof(float);
        DataCopyParams colMaxCopyParams{1, copyLen, 0, 0};
        DataCopyPad(colMaxLocal, colMaxGlobal, colMaxCopyParams, padParams);
        inQueueColMax.template EnQue(colMaxLocal);
        // Copy Bias
        if (tiling.tilingData.isBias == 1) {
            LocalTensor <half> biasLocal = inQueueBias.template AllocTensor<half>();
            DataCopyParams biasCopyParams{1, static_cast<uint16_t>(copyLen / 2), 0, 0};
            DataCopyPad(biasLocal, biasGlobal, biasCopyParams, padParams);
            inQueueBias.template EnQue(biasLocal);
        }
    }

    template<uint16_t bufferNum>
    __aicore__ inline void DequantizeVectorNonAligned<bufferNum>::Compute(uint32_t rowCount, uint32_t dataLen)
    {
        LocalTensor<int32_t> ptrALocal = inQueuePtrA.template DeQue<int32_t>();
        LocalTensor<float> ptrAFloattemp = aMatrixFloatBuf.Get<float>();
        Cast(ptrAFloattemp, ptrALocal, RoundMode::CAST_NONE, dataLen);
        pipe_barrier(PIPE_V);
        inQueuePtrA.template FreeTensor(ptrALocal);

        LocalTensor<float> rowMaxLocal = inQueueRowMax.template DeQue<float>();
        LocalTensor<float> colMaxLocal = inQueueColMax.template DeQue<float>();
        LocalTensor<float> rowColProfuctLocal = rowColProductFloatBuf.Get<float>();
        set_flag(PIPE_V, PIPE_S, EVENT_ID0);
        wait_flag(PIPE_V, PIPE_S, EVENT_ID0);
        for (int32_t i = 0; i < rowCount; i++){
            float rowMax = rowMaxLocal.GetValue(i);
            for (int32_t j = 0; j < tiling.tilingData.N; j++){
                rowColProfuctLocal.SetValue(i * tiling.tilingData.N + j, rowMax * colMaxLocal.GetValue(j));
            }
        }
        set_flag(PIPE_S, PIPE_V, EVENT_ID0);
        wait_flag(PIPE_S, PIPE_V, EVENT_ID0);
        ...
        inQueueRowMax.template FreeTensor(rowMaxLocal);
        inQueueColMax.template FreeTensor(colMaxLocal);
        Mul(ptrAFloattemp, ptrAFloattemp, rowColProfuctLocal, dataLen);
        pipe_barrier(PIPE_V);

        float factor = 1.0f / (127.0f * 127.0f);
        Muls(ptrAFloattemp, ptrAFloattemp, factor, dataLen);
        pipe_barrier(PIPE_V);
        ...
        if (tiling.tilingData.isBias == 1) {
            ...
            Cast(biasFloatLocal, biasLocal, RoundMode::CAST_NONE, 16);

            set_flag(PIPE_V, PIPE_S, EVENT_ID0);
            wait_flag(PIPE_V, PIPE_S, EVENT_ID0);
            for (int32_t i = 0; i < rowCount; i++) {
                for (int32_t j = 0; j < tiling.tilingData.N; j++) {
                    biasFloatLocal.SetValue(i * tiling.tilingData.N + j, biasFloatLocal.GetValue(j));
                }
            }
            set_flag(PIPE_S, PIPE_V, EVENT_ID0);
            wait_flag(PIPE_S, PIPE_V, EVENT_ID0);
            ...
            inQueueBias.template FreeTensor(biasLocal);
            Add(ptrAFloattemp, ptrAFloattemp, biasFloatLocal, dataLen);
            pipe_barrier(PIPE_V);
        }

        LocalTensor<half> outALocal = outQueueOutA.template AllocTensor<half>();
        Cast(outALocal, ptrAFloattemp, RoundMode::CAST_NONE, dataLen);
        pipe_barrier(PIPE_V);
    ...
    template<uint16_t bufferNum>
    __aicore__ inline void DequantizeVectorNonAligned<bufferNum>::CopyOut(uint32_t rowOffset, uint32_t rowCount, uint32_t dataLen)
    {
        // copy OutA
        LocalTensor<half> outALocal = outQueueOutA.template DeQue<half>();
        DataCopy(outAGlobal[rowOffset * tiling.tilingData.N], outALocal, dataLen);
        outQueueOutA.template FreeTensor(outALocal);
    }

    // DequantizeVector
    template<uint16_t bufferNum>
    class DequantizeVector {
    public:
        __aicore__ inline DequantizeVector() {}
        __aicore__ inline ~DequantizeVector() {}
        __aicore__ inline void Init(GM_ADDR ptrA_gm, GM_ADDR ptrRow_gm, GM_ADDR ptrCol_gm, GM_ADDR bias_gm,
                                    GM_ADDR outA_gm, GM_ADDR tiling_gm);
        __aicore__ inline void Process();
    private:
        DequantizeTilingData tiling;
        DequantizeVectorAligned<bufferNum> aligned;
        DequantizeVectorNonAligned<bufferNum> nonaligned;
    };

    template<uint16_t bufferNum>
    __aicore__ inline void DequantizeVector<bufferNum>::Init(GM_ADDR ptrA_gm, GM_ADDR ptrRow_gm, GM_ADDR ptrCol_gm, GM_ADDR bias_gm,
                                                             GM_ADDR outA_gm, GM_ADDR tiling_gm)
    {
        auto tempTilingGm = (__gm__ DequantizeTilingData*)tiling_gm;
        tiling.is32BAligned = tempTilingGm->is32BAligned;

        if (tiling.is32BAligned == 1){
            aligned.Init(ptrA_gm, ptrRow_gm, ptrCol_gm, bias_gm, outA_gm, tiling_gm);
        } else{
            nonaligned.Init(ptrA_gm, ptrRow_gm, ptrCol_gm, bias_gm, outA_gm, tiling_gm);
        }
    }

    template<uint16_t bufferNum>
    __aicore__ inline void DequantizeVector<bufferNum>::Process() {
        if (tiling.is32BAligned == 1){
            aligned.Process();
        } else{
            nonaligned.Process();
...
__global__ __aicore__ void row_col_dequant(GM_ADDR ptrA_gm, GM_ADDR ptrRow_gm, GM_ADDR ptrCol_gm, GM_ADDR bias_gm,
                                           GM_ADDR outA_gm, GM_ADDR tiling)
{
    DequantizeTilingData tempTilingGm;
    if (tempTilingGm.isDoubleBuffer == 1) {
        row_col_dequant_kernel::DequantizeVector<2> op;
        op.Init(ptrA_gm, ptrRow_gm, ptrCol_gm, bias_gm, outA_gm, tiling);
        op.Process();
    } else {
        row_col_dequant_kernel::DequantizeVector<1> op;
        op.Init(ptrA_gm, ptrRow_gm, ptrCol_gm, bias_gm, outA_gm, tiling);
        op.Process();
    }
}
